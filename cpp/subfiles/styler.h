...
// 	friend std::fstream &operator<<(std::fstream &sObj, const cssRules & ru) {
// 		for (auto rule : ru.rules) {
// 			sObj << rule << std::endl;
// 		}
// 		return sObj;
// 	};
...
// 	std::string pat = elname + "." + elname[0];
// 	//std::string klass;
// 	std::regex klass(".(\\w|\\d)+");
...
private:
	...
	std::vector<std::vector<std::string>>impSp, impP;
};
